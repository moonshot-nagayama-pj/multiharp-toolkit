...
fn _mhtk_rs(_py: Python, m: &PyModule) -> PyResult<()> {
    with_mhlib!(m.add_function(wrap_pyfunction!(get_library_version, m)?)?);
    with_mhlib!(m.add_function(wrap_pyfunction!(open_device, m)?)?);
    with_mhlib!(m.add_function(wrap_pyfunction!(initialize, m)?)?);
    with_mhlib!(m.add_function(wrap_pyfunction!(close_device, m)?)?);
    with_mhlib!(m.add_function(wrap_pyfunction!(get_serial_number, m)?)?);
    with_mhlib!(m.add_function(wrap_pyfunction!(get_number_of_input_channels, m)?)?);
    with_mhlib!(m.add_function(wrap_pyfunction!(set_sync_divider, m)?)?);
    with_mhlib!(m.add_function(wrap_pyfunction!(set_sync_edge_trigger, m)?)?);
    with_mhlib!(m.add_function(wrap_pyfunction!(set_sync_channel_offset, m)?)?);
    with_mhlib!(m.add_function(wrap_pyfunction!(set_input_edge_trigger, m)?)?);
    with_mhlib!(m.add_function(wrap_pyfunction!(set_input_channel_enable, m)?)?);
    with_mhlib!(m.add_function(wrap_pyfunction!(set_input_channel_offset, m)?)?);
    with_mhlib!(m.add_function(wrap_pyfunction!(set_binning, m)?)?);
    with_mhlib!(m.add_function(wrap_pyfunction!(set_offset, m)?)?);
    with_mhlib!(m.add_function(wrap_pyfunction!(get_resolution, m)?)?);
    with_mhlib!(m.add_function(wrap_pyfunction!(get_sync_rate, m)?)?);
    with_mhlib!(m.add_function(wrap_pyfunction!(get_count_rate, m)?)?);
    with_mhlib!(m.add_function(wrap_pyfunction!(get_warnings, m)?)?);
    with_mhlib!(m.add_function(wrap_pyfunction!(start_measurement, m)?)?);
    with_mhlib!(m.add_function(wrap_pyfunction!(stop_measurement, m)?)?);
    with_mhlib!(m.add_function(wrap_pyfunction!(read_fifo, m)?)?);
    with_mhlib!(m.add_function(wrap_pyfunction!(is_measurement_running, m)?)?);
